...
void Camera::Update()
{
	...
	if (this->following)
	{
		cameraX = this->following->getX();
		cameraY = this->following->getY();
...
void Camera::Follow(Ninja *following)
{
...
void Camera::SetTransform(CGame *Device) const
{
	Device->GetDirect3DDevice()->SetTransform(D3DTS_PROJECTION, &orthographicMatrix);
	Device->GetDirect3DDevice()->SetTransform(D3DTS_WORLD, &identityMatrix);
	Device->GetDirect3DDevice()->SetTransform(D3DTS_VIEW, &ViewMatrix);
}